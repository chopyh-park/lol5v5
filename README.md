# -*- coding: utf-8 -*-
"""
LoL 5v5 팀 매칭 스크립트 (v7.7)
================================

v7.7에서는 기존 v7.6을 기반으로 다음과 같은 변경을 적용했다.

* **윤영 라인 제한 정리**: 더 이상 Top/Jg 두 가지 경우를 모두 탐색하지 않고,
  윤영을 항상 Top에 고정한다. 따라서 `forced_roles` 목록을 ['Top']으로 줄였다.
  메인 루프도 이에 맞춰 수정되어 윤영이 Top으로만 배정된다.

* **구조 유지**: 선형 MMR 계산, λ·w 기반 오프롤 페널티, DFS 배치, 금지 매치업,
  NOT_SAME_TEAM 등의 기본 설계는 v7.6과 동일하다.
...
TOP_N_RESULTS = 200    # 탐색 시 유지할 상위 후보 개수
...
# 기본 금지 매치업과 NOT_SAME_TEAM 목록
_default_forbid_matchups    = {('윤영','우재'), ('용훈','혜원')}
_default_not_same           = {('고래','윤영')}

# 기본 타겟 티어/라인 오버라이드. 규칙 파일이 없으면 빈 딕셔너리로 사용된다.
_default_player_pair_target: Dict[Tuple[str, str], str] = {}
_default_role_overrides: Dict[Tuple[str, str], str] = {}

# 규칙 파일 로드
_rules_data: Dict[str, any] = load_rules()

# 윤영의 라인 강제 목록. v7.7에서는 Top만 강제한다.
forced_roles: List[str] = ['Top']
...
def _norm_role(r: str) -> str:
    ...
    if r in ('top','탑'):
        return 'Top'
    if r in ('jg','jungle','정글'):
        return 'Jg'
    if r in ('mid','미드'):
        return 'Mid'
    if r in ('adc','bot','원딜','바텀'):
        return 'Adc'
    if r in ('sup','support','서폿','서포터'):
        return 'Sup'
...
###############################################################################
# 상태 저장/로드 (미사용이지만 기존 인터페이스 유지를 위해 남겨둔다)
def load_state() -> dict:
...
if __name__ == '__main__':
    ...
    # 윤영을 Top으로 강제해 DFS 탐색을 한 번만 수행한다.
